...
def run_categorize(categories_path: str, rules_path: str, inputs: List[str], out_dir: Optional[str]) -> int:
    ...
    updated_merchants: Dict[str, str] = {}  # merchant -> category decided (for exact rules)
...
def load_categories(path: Path) -> List[str]:
    if not path.exists():
        return []
    cats = [line.strip() for line in path.read_text(encoding="utf-8").splitlines() if line.strip()]
...
def write_monarch_csv(src_csv: Path, rows: List[Dict[str, str]], out_dir: Optional[str]) -> Path:
    out_dir_p = Path(out_dir) if out_dir else src_csv.parent
    out_dir_p.mkdir(parents=True, exist_ok=True)
    stem = src_csv.stem.replace(".activity", "") if src_csv.name.endswith("_activity.csv") else src_csv.stem
...
def transform_rows(rows: List[Dict[str, str]], rules: RuleSet, cats: List[str]) -> Tuple[List[Dict[str, str]], Dict[str, str]]:
    out: List[Dict[str, str]] = []
    updated_merchants: Dict[str, str] = {}  # merchant -> category

    for r in rows:
        date, amount = pick(r, ["Date", "Posted Date", "Transaction Date"]), pick(r, ["Amount"])
        ...
        if existing_cat:
            ...
            payee = normalized_payee(desc, existing_cat, rules)  # will respect exact rule if found
...
def monarch_row(date: str, payee: str, category: str, notes: str, amount: str) -> Dict[str, str]:
    return {"Date": date, "Payee": payee, "Category": category, "Notes": notes or "", "Amount": amount}
...
def update_rules_exact(rules: RuleSet, merchant_to_category: Dict[str, Optional[str]]) -> None:
